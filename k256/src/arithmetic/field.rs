...
impl FieldElement {
    ...
    /// Computes a candidate for the square root of `self` mod `p`.
    ///
    ...
    ///
    /// For a safe interface that verifies the candidate, use [`Self::sqrt()`].
    fn compute_sqrt(&self) -> FieldElement {
        ...
        // The final result is then assembled using a sliding window over the blocks.
        x223.pow2k(23).mul(&x22).pow2k(6).mul(&x2).pow2k(2)
    }

    /// Computes the square root of `self` without any final verification.
    ///
    /// In a native (non-zkVM) environment, this function simply calls
    /// [`Self::compute_sqrt`]. In a zkVM environment, it uses an unconstrained
    /// block and hinting mechanism before reading back the result. Because no
    /// checks are performed to verify that the output is indeed a valid square
    /// root of `self`, the caller must ensure that `self` is a quadratic residue
    /// if correctness is required.
    ///
    /// For a safe interface that verifies the result, use [`Self::sqrt()`].
    fn sqrt_unchecked(&self) -> Self {
        // In zkVM environment: do unconstrained sqrt, hint, and read back
        #[cfg(all(target_os = "zkvm", target_vendor = "succinct"))]
        let res = {
            sp1_lib::unconstrained! {
                let tmp = self.compute_sqrt();
                sp1_lib::io::hint_slice(tmp.to_bytes().as_slice());
            }
            return Self::from_bytes(FieldBytes::from_slice(&sp1_lib::io::read_vec())).unwrap();
        };

        // In non-zkVM (native) environment: compute directly
        self.compute_sqrt()
    }
